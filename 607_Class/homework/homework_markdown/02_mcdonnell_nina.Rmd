...
After viewing plots of flipper length ~ mass for individual penguins from the whole dataset, I can see that there is a positive realtionship between the two variables within each group (species_island) sampled. However, this does not clarify the potential relationship between flipper length and body mass across species from 3b. There may be many other species that do not fit the trend of greater species body size = greater species flipper length (say, a species with very short flippers and large body mass), even if the two traits do have a positive relationship within their own populations.  Understanding the relationship between the traits within populations does not shed any light on the pattern observed across species. All we can say is that within each species_island group that was sampled, there was a positive relationship between body mass and flipper length.

``` {r 3c}

new_penguins4 <- new_penguins %>% #made new data frame without averaged mass and flipper length for each species_island
  group_by(species_island) %>% #grouping penguins by species_island
...
## *Extra credit:* submitted via GitHub pull request. 
### Repository: https://github.com/ninamcdonnell/biol607_homework
